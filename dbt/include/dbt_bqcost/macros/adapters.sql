...
{% macro dbt_bqcost__alter_column_type(relation,column_name,new_column_type) -%}
...
{% macro dbt_bqcost__check_schema_exists(information_schema,schema) -%}
...
{% macro dbt_bqcost__create_schema(relation) -%}
...
{% macro dbt_bqcost__drop_relation(relation) -%}
...
{% macro dbt_bqcost__drop_schema(relation) -%}
...
{% macro dbt_bqcost__get_columns_in_relation(relation) -%}
...
{% macro dbt_bqcost__list_relations_without_caching(schema_relation) -%}
'''creates a table of relations withough using local caching.'''
{% endmacro %}

{% macro dbt_bqcost__list_schemas(database) -%}
...
{% macro dbt_bqcost__rename_relation(from_relation, to_relation) -%}
...
{% macro dbt_bqcost__truncate_relation(relation) -%}
...
{% macro dbt_bqcost__current_timestamp() -%}
